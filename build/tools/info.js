...
NO REQUIERE dispositivo activo para búsquedas, pero sí para nowPlaying/state/queue.`,
    ...
    ejecutar: async (args, _extra) => {
        ...
        switch (accion) {
            case 'search': {
                if (!consulta || !tipo)
                    return { content: [{ type: 'text', text: 'Error: Requiere consulta y tipo' }] };
                const res = await ejecutarPeticion((api) => api.search(consulta, [tipo], undefined, limite));
                let texto = '';
            ...
            case 'nowPlaying': {
                ...
                if (!actual?.item)
                    return { content: [{ type: 'text', text: '🔇 Nada reproduciéndose' }] };
                if (!esCancion(actual.item))
                    return { content: [{ type: 'text', text: '🎙️ Reproduciendo podcast' }] };
                const { item } = actual;
            ...
            case 'devices': {
                ...
                if (!d?.devices?.length)
                    return { content: [{ type: 'text', text: '📵 Sin dispositivos. Abre Spotify.' }] };
            ...
            case 'queue': {
                ...
                if (!cola?.queue?.length)
                    return { content: [{ type: 'text', text: '📭 Cola vacía' }] };
                let actual = '';
            ...
            case 'history': {
                const h = await ejecutarPeticion((api) => api.player.getRecentlyPlayedTracks(limite));
                if (!h.items.length)
                    return { content: [{ type: 'text', text: '📭 Sin historial' }] };
            ...
            case 'saved': {
                const g = await ejecutarPeticion((api) => api.currentUser.tracks.savedTracks(limite, offset));
                if (!g.items.length)
                    return { content: [{ type: 'text', text: '📭 Sin guardadas' }] };
            ...
            case 'playlists': {
                const pl = await ejecutarPeticion((api) => api.currentUser.playlists.playlists(limite));
                if (!pl.items.length)
                    return { content: [{ type: 'text', text: '📭 Sin playlists' }] };
            ...
            case 'playlistTracks': {
                if (!id)
                    return { content: [{ type: 'text', text: 'Error: Requiere id de playlist' }] };
                const c = await ejecutarPeticion((api) => api.playlists.getPlaylistItems(id, undefined, undefined, limite));
                if (!c.items?.length)
                    return { content: [{ type: 'text', text: '📭 Playlist vacía' }] };
            ...
            case 'albumTracks': {
                if (!id)
                    return { content: [{ type: 'text', text: 'Error: Requiere id de álbum' }] };
                const album = await ejecutarPeticion((api) => api.albums.get(id));
                if (!album?.tracks?.items?.length)
                    return { content: [{ type: 'text', text: '📭 Sin canciones' }] };
            ...
            case 'artistTop': {
                if (!id)
                    return { content: [{ type: 'text', text: 'Error: Requiere id de artista' }] };
                const top = await ejecutarPeticion((api) => api.artists.topTracks(id, mercado));
            ...
            case 'topTracks': {
                const top = await ejecutarPeticion((api) => api.currentUser.topItems('tracks', periodo, limite));
                if (!top?.items?.length)
                    return { content: [{ type: 'text', text: '📭 Sin datos' }] };
            ...
            case 'topArtists': {
                const top = await ejecutarPeticion((api) => api.currentUser.topItems('artists', periodo, limite));
                if (!top?.items?.length)
                    return { content: [{ type: 'text', text: '📭 Sin datos' }] };
                const periodos = { short_term: '4 semanas', medium_term: '6 meses', long_term: 'siempre' };
            ...
            case 'state': {
                ...
                if (!e)
                    return { content: [{ type: 'text', text: '📵 Sin sesión activa' }] };
